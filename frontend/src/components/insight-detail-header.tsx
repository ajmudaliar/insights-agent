      ...
      <div className="space-y-5">
        ...
        <Collapsible open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
          <CollapsibleTrigger asChild>
            ...
            >
              <ChevronDown
                className={`h-3 w-3 transition-transform duration-200 ${isDetailsOpen ? "rotate-180" : ""}`}
              />
          ...
          <CollapsibleContent className="space-y-4 pt-2">
            ...
            {config.clustering_focus && (
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground/70 leading-relaxed pl-5">{config.clustering_focus}</p>
              </div>
            )}
              ...
              <div className="space-y-2">
                ...
                <div className="pl-5 flex flex-wrap gap-2">
                  {config.extract_features.map((feature, idx) => (
                    <Badge key={idx} variant="outline" className="h-5 text-[10px] px-2 py-0">
                      {feature}
                    </Badge>
                  ))}
                </div>
                  ...
                  {config.attributes.map((attr, idx) => (
                    <div key={idx} className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-medium text-foreground/90">{attr.name}</span>
